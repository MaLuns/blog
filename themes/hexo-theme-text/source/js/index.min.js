...
$(function(){var _setTimeout;var app={setting:localStorage.getItem('setting')?JSON.parse(localStorage.getItem('setting')):{},initSettingEvent:function initSettingEvent(){var root=document.documentElement;$("#back-top").on("click",function(){window.scrollTo({top:0,behavior:"smooth"});});$("#lightSwitch").on('click',function(ev){var that=$(ev.delegateTarget);var checked=that.hasClass('theme-toggle--checked');var funKey=checked?'remove':'add';that[funKey+'Class']('theme-toggle--checked');root.classList[funKey]('dark');app.setSetting('lightSwitch',checked);var bg=$('#light-switch-mark');if(_setTimeout){bg.removeClass('active');clearTimeout(_setTimeout);}bg.addClass('active');_setTimeout=setTimeout(function(){bg.removeClass('active');},300);});},LoadMore:function LoadMore(){if(!!window.IndexConfig&&!IndexConfig.loadMore&&IndexConfig.current<IndexConfig.total){IndexConfig.loadMore=true;$("#loader").addClass('loading').removeClass('loaded');$.ajax({url:IndexConfig.url+'page/'+ ++IndexConfig.current,success:function success(res){var result=$(res).find("article");$("#posts").append(result.fadeIn(5000));},error:function error(){--IndexConfig.current;},complete:function complete(){IndexConfig.loadMore=false;$("#loader").addClass('loaded').removeClass('loading');}});}},initSetting:function initSetting(){var setting=this.setting;for(var key in setting){switch(key){case'lightSwitch':if(setting[key]===false||setting[key]==='false'){$("#lightSwitch").addClass('theme-toggle--checked');}else{$("#lightSwitch").removeClass('theme-toggle--checked');}break;case'filterColor':case'radiusSlider':case'boxShadowBlur':case'fontFamily':/* case 'themeColorPicker':
