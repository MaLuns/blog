...
import './com-popover.js';

class ComOptionGroup extends HTMLElement {
    constructor() {
        super();
...
if(!customElements.get('com-optgroup')){
    customElements.define('com-optgroup', ComOptionGroup);
}

class ComOption extends HTMLElement {
    ...
    constructor() {
        ...
        <com-button id="option" class="option" type="flat" ${this.disabled?"disabled":""}><slot></slot></com-button>
        `
    }
...
if(!customElements.get('com-option')){
    customElements.define('com-option', ComOption);
}

export default class ComSelect extends HTMLElement {
    ...
    constructor() {
        ...
        :host(:focus-within) com-popover,:host(:active) com-popover{ 
            z-index: 2;
        }
        com-tips{
            display:block;
        ...
        :host([empty]) com-popcon::before{
            display:block;
        }
        </style>
        <style id="filter"></style>
        <com-popover id="root" ${this.search?"accomplish":""}>
            <com-tips id="tip" type="error">
                <${this.search?'com-input':'com-button'} id="select" debounce="200" readonly ${this.disabled? "disabled" : ""} ${this.type?("type="+this.type):""}>${this.search?"":'<span id="value"></span>'}<svg class="arrow" viewBox="0 0 1024 1024"><path d="M884 256h-75c-5.1 0-9.9 2.5-12.9 6.6L512 654.2 227.9 262.6c-3-4.1-7.8-6.6-12.9-6.6h-75c-6.5 0-10.3 7.4-6.5 12.7l352.6 486.1c12.8 17.6 39 17.6 51.7 0l352.6-486.1c3.9-5.3 0.1-12.7-6.4-12.7z"></path></svg></${this.search?'com-input':'com-button'}>
            </com-tips>
            <com-popcon id="options">
                <slot id="slot"></slot>
            </com-popcon>
        </com-popover>
        `
    ...
    connectedCallback() {
        ...
        this.options.addEventListener('click',(ev)=>{
            this.focus();
            const item = ev.target.closest('com-option');
            if(item){
                this.nativeclick = true;
        ...
        this.options.addEventListener('close',(ev)=>{
            if(this.search){
                ...
                this.nodes = [...this.querySelectorAll(`com-option:not([disabled])`)];
                this.filter.textContent = '';
                this.empty = false;

            }
            const place = this.querySelector(`com-option[focusin]`);
            const current = this.querySelector(`com-option[selected]`);
            if(place){
                place.focusin = false;
            }
            if(current){
        ...
        if(this.search){
            ...
            this.select.addEventListener('input',(ev)=>{
                ...
                if(value===""){
                    ...
                    this.nodes = [...this.querySelectorAll(`com-option[key*="${value}" i]:not([disabled])`)];
                    this.filter.textContent = `
                    :host([search]) ::slotted(com-option:not([key*="${value}" i]))
                    {
                        display:none;
                    }
                    `
    ...
    set value(value) {
        ...
        if (value !== this.value) {
            this.$value = value;
            const pre = this.querySelector(`com-option[selected]`);
            if(pre){
                pre.selected = false;
                pre.focusin = false;
            }
            const cur = this.querySelector(`com-option[value="${value}"]`)||this.querySelector(`com-option`);
...
if (!customElements.get('com-select')) {
    customElements.define('com-select', ComSelect);
}