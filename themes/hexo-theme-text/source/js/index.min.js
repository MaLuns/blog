...
$(function(){var app={setting:localStorage.getItem('setting')?JSON.parse(localStorage.getItem('setting')):{},initSettingEvent:function initSettingEvent(){var root=document.documentElement;$("#back-top").on("click",function(){window.scrollTo({top:0,behavior:"smooth"});});$("#lightSwitch").on('click',function(ev){var that=$(ev.delegateTarget);var checked=that.hasClass('theme-toggle--checked');if(checked){that.removeClass('theme-toggle--checked');}else{that.addClass('theme-toggle--checked');}root.classList[checked?'add':'remove']('dark');app.setSetting('lightSwitch',checked);});},LoadMore:function LoadMore(){if(!!window.IndexConfig&&!IndexConfig.loadMore&&IndexConfig.current<IndexConfig.total){IndexConfig.loadMore=true;$("#loader").addClass('loading').removeClass('loaded');$.ajax({url:IndexConfig.url+'page/'+ ++IndexConfig.current,success:function success(res){var result=$(res).find("article");$("#posts").append(result.fadeIn(5000));},error:function error(){--IndexConfig.current;},complete:function complete(){IndexConfig.loadMore=false;$("#loader").addClass('loaded').removeClass('loading');}});}},initSetting:function initSetting(){var setting=this.setting;for(var key in setting){switch(key){case'lightSwitch':$("#lightSwitch").attr('checked',setting[key]);break;case'filterColor':case'radiusSlider':case'boxShadowBlur':case'fontFamily':/* case 'themeColorPicker':
                        document.getElementById(key).value = setting[key].val;
                        break */default:break;}}},setSetting:function setSetting(key,value){this.setting[key]=value;localStorage.setItem('setting',JSON.stringify(this.setting));},scroll:function scroll(){var root=document.documentElement;var htmlHeight=document.body.scrollHeight||root.scrollHeight;var clientHeight=$(window).height()+100;var scrollTop=document.body.scrollTop||root.scrollTop;if(scrollTop+clientHeight>htmlHeight){app.LoadMore();}if(scrollTop>100){$(".layout-header").addClass('fixed');}else{$(".layout-header").removeClass('fixed');}if(scrollTop>500){$(".backtop").addClass("show");}else{$(".backtop").removeClass("show");}}};$(window).scroll(app.scroll);app.initSetting();app.initSettingEvent();app.scroll();mediumZoom('article img',{margin:24,background:'#00000077',scrollOffset:0});});
