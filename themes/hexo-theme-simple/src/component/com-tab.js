...
class ComTabContent extends HTMLElement {
...
if(!customElements.get('com-tab-content')){
    customElements.define('com-tab-content', ComTabContent);
}

export default class ComTab extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this.slots.addEventListener('slotchange', ()=>{
            ...
            slots.forEach((item,index)=>{
                if( item.tagName === 'com-TAB-CONTENT' ){
                    if(item.key===null){
                        item.key = index;
                    }
                    html += `<com-button class="nav-item ${item.key===this.activekey?'active':''}" icon=${item.icon} ${item.disabled!==null?"disabled":""} data-key=${item.key}>${item.label}</com-button>`;
                }
            })
        ...
        this.nav.addEventListener('click',(ev)=>{
            const item = ev.target.closest('com-button');
            if(item){
    ...
    attributeChangedCallback (name, oldValue, newValue) {
        if( name == 'activekey' && this.tab){
            ...
            ::slotted(com-tab-content:not([key="${this.activekey}"])){
                height:0;
                overflow:visible;
            }
            `
...
if(!customElements.get('com-tab')){
    customElements.define('com-tab', ComTab);
}